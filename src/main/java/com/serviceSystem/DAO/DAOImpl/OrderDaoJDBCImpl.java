...
public class OrderDaoJDBCImpl implements OrderDAO {
    ...
    private final String ORDERS_BY_TABLE_ID = SELECT_ALL + " where o.table_id=?";
    private final String UPDATE_ORDER = "UPDATE restaurantdb.orders set table_id = ?, worker_id = ? , status = CAST(? as restaurantdb.status) where id = ?";
    private final String UPDATE_ORDER_WITHOUT_WORKER = "UPDATE restaurantdb.orders set table_id = ?,status = CAST(? as restaurantdb.status) where id = ?";
    private final String GET_ACTIVE_BY_CLIENT_ID = SELECT_ALL + " where client_id = ? and o.status != CAST(\'COMPLETED\' as restaurantdb.status)" +
            " and status != cast(\'CANCELLED\' as restaurantdb.status)";
    private final String GET_WITH_FREE_TABLE = SELECT_ALL + " where t.is_free = true and status = CAST(\'NOT_TAKEN\' as restaurantdb.status)";
...
//                    }
    ...
    public void update(Order order) {
        ...
        String sqlQuery = order.getWorker().getId() == 0 ? UPDATE_ORDER_WITHOUT_WORKER : UPDATE_ORDER;
        try (Connection connection = HikariCP.getConnection();
             PreparedStatement statement = connection.prepareStatement(sqlQuery);) {
            try {
                connection.setAutoCommit(false);
                statement.setInt(1, order.getTable().getId());
                if (order.getWorker().getId() == 0) {
...
//        List<Order> orders = new ArrayList<Order>();
    ...
    public List<Order> getByTable(int tableId) {
        ...
        List<Order> orders = null;
        try (Connection connection = HikariCP.getConnection();
             PreparedStatement statementForOrders = connection.prepareStatement(ORDERS_BY_TABLE_ID);
             PreparedStatement statementForDishes = connection.prepareStatement(DISHES_FROM_ORDER);) {
    ...
    public List<Order> getNotTakenWithFreeTable() {
        ...
        try (Connection connection = HikariCP.getConnection();
            ...
            ResultSet setOfOrders = statementForOrders.executeQuery(GET_WITH_FREE_TABLE);
            orders = EntityBuilder.buildOrders(setOfOrders, null);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return orders;
